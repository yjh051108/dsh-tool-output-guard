...
const EMPTY_TEXT: ContentBlockLike = { type: 'text', text: '' }
...
function isBlockLike(value: unknown): value is ContentBlockLike {
  return typeof value === 'object' && value !== null && !Array.isArray(value)
    && typeof (value as ContentBlockLike).type === 'string' && (value as ContentBlockLike).type !== ''
}

/** 归一单个条目；`changed=false` 表示原样可用（引用不变）。 */
function normalizeBlock(entry: unknown, prefix: string, fixes: string[]): { block: ContentBlockLike; changed: boolean } {
  ...
  if (entry.type === 'tool-result') {
    ...
    if (inner.changed) return { block: { ...entry, content: inner.list }, changed: true }
  }
  return { block: entry, changed: false }
}

/** 归一块数组（tool-result 的 content 走这里）；合法则原样返回同一引用。 */
function normalizeList(value: unknown, prefix: string, fixes: string[]): { list: ContentBlockLike[]; changed: boolean } {
  ...
  if (!changed) return { list: value as ContentBlockLike[], changed: false }
 ...
 */
export function normalizeToolContent(content: unknown): NormalizedContent {
  const fixes: string[] = []
